...
| `cosyvoice` | `cosyvoice:<spk_id>` | CosyVoice runtime API at `cosyvoice.url` (auto-start via `cosyvoice.command`) |
...
Then `kotoba play ...` will probe the endpoint and launch the runtime automatically when needed.
